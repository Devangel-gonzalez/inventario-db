...
public class Database {
    private static final String URL = "jdbc:sqlite:InventarioBD.db"; // Nueva URL para la base de datos
    ...
    private void insertDefaultUser(String nombre, String passPlain, String rol) throws SQLException {
        ...
        try (Connection c = connect(); PreparedStatement ps = c.prepareStatement(check)) {
            ...
            try (ResultSet rs = ps.executeQuery()) { // (Corregido) ResultSet dentro del try para asegurar cierre
                if (!rs.next()) {
    ...
    public List<Producto> listProductos() {
        ...
        String sql = """
                ...
                FROM productos p
                LEFT JOIN almacenes a ON p.almacen_id = a.id"""; // (Corregido) Mejor legilibilidad
    ...
    public int insertProducto(Producto prod, String usuario) {
        String sql = """
                INSERT INTO productos(
                    ...
                    fecha_hora_ultima_modificacion,
                    ultimo_usuario_en_modificar
                ) VALUES(?,?,?,?,?,?,?,?)"""; // (Corregido) SQL para insertar producto con campos correctos
        ...
        try (Connection c = connect();
            ...
            ps.setString(6, LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)); // fecha_hora_creacion
            ps.setString(7, LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)); // fecha_hora_ultima_modificacion
